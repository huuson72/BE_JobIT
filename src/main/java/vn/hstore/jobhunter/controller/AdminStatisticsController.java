...
public class AdminStatisticsController {
    ...
    @GetMapping("/recent")
    @ApiMessage("Get recent statistics")
    public ResponseEntity<?> getRecentStatistics() {
        try {
            AdminStatisticsDTO statistics = adminStatisticsService.getRecentStatistics();

            RestResponse<AdminStatisticsDTO> response = new RestResponse<>();
            response.setStatusCode(200);
            response.setError(null);
            response.setMessage("Lấy thống kê gần đây thành công");
            response.setData(statistics);
    ...
    @GetMapping("/trends")
    @ApiMessage("Get trend statistics")
    public ResponseEntity<?> getTrendStatistics() {
        try {
            AdminStatisticsDTO statistics = adminStatisticsService.getTrendStatistics();

            RestResponse<AdminStatisticsDTO> response = new RestResponse<>();
            response.setStatusCode(200);
            response.setError(null);
            response.setMessage("Lấy thống kê xu hướng thành công");
            response.setData(statistics);
