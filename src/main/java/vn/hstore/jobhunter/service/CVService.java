...
public class CVService {
    ...
    public CV createCV(CVRequestDTO cvRequest) {
        User user = userRepository.findById(cvRequest.getUserId())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy user với ID: " + cvRequest.getUserId()));

        Job job = null;
        if (cvRequest.getJobId() != null) { // Kiểm tra jobId trước
            job = jobRepository.findById(cvRequest.getJobId())
                    .orElseThrow(() -> new RuntimeException("Không tìm thấy job với ID: " + cvRequest.getJobId()));
        }
        ...
        cv.setUser(user);
        cv.setJob(job); // job có thể là null
